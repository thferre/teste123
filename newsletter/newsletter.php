...
class Newsletter extends Module
{
	...
	public function __construct()
	{
		...
		if ($this->id)
		{
			$this->file = '_'.Configuration::get('PS_NEWSLETTER_RAND').'.csv';
	...
	public function CronNews()
	{
		...
		if ($result)
		{
			if (!$nb = count($result))
				$this->html .= $this->displayError($this->l('No customers found with these filters!'));
			elseif ($fd = @fopen('/var/www/webroot/ROOT/allinmailing/'.date("d.m.Y-H:i:s").$this->file, 'w'))
			{
				...
				$this->html .= $this->displayError($this->l('Error: Write access limited').' '.dirname(__FILE__).'/'.$this->file.' !');
		}
	}
	...
	public function getCustomers()
	{
		...
		{
			$dbquery = new DbQuery();
			$dbquery->select('c.`firstname`, c.`lastname`, c.`email`');
	...
	public function getBlockNewsletter()
	{
