...
class PagSeguroPro extends PaymentModule {
    ...
    public function hookDisplayAdminOrder()
    {
		...
		if (Tools::isSubmit('cancelarPedidoPagSeguro')) {
			if($transaction->status < 3) {
				...
				$this->saveLog('cancelamento', 'pedido', json_encode($info), json_encode($retorno), '');
				if ($retorno['error'] == 0) {
					$data = array(
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'col' => 5,
						'multiple' => true,
                        'desc' => $this->l('Escolha os meios de pagamento que serão ativos em sua loja.').' <br /> <b>OBS.:</b> <br />Consulte antes se o meio de pagamento está ativo em sua conta <b><a href="https://pagseguro.uol.com.br/preferences/receiving.jhtml" target="_blank" title="Configurar Meios de Pagamento">PagSeguro</a></b>. <br />Por padrão, para Boleto Bancário, é cobrada uma taxa de emissão no valor de R$ 1,00, mas você pode negocias suas taxas e tarifas para todas as formas de pagamento.',
						'options' => array(
							'query' => array(
								array(
									'id' => 'cartao',
	...
	public function updatePagSeguroData($data)
	{
		if(!$this->getOrderData($data['cod_transacao'], 'cod_transacao')) {
			...
			if (!Db::getInstance()->execute($updateQuery)) {
				$this->saveLog('error', 'update', $updateQuery, 'Pedido nao atualizado no banco.');
				return false;
			}
			return true;
		}
	...
	public function insertPagSeguroData($data)
	{
		if($this->getOrderData($data['cod_transacao'], 'cod_transacao')) {
			...
			if (!Db::getInstance()->execute($insQuery)) {
				$this->saveLog('error', 'insert', $insQuery, 'Pedido nao inserido no banco.');
				return false;
			}
			return true;
		}
	...
	{
		$retorno = $this->curl_send('POST',$this->urls['session'], $this->credentials, 10);
    ...
    public function processarCartao($dados_form) 
	{
		...
		$retorno = $this->curl_send('POST',$this->urls['transaction'], $this->ps_params, 30);
	...
	{
		$retorno = $this->curl_send('GET',$this->urls['notification'].'/'.$code, $this->credentials, 30);
        if ($retorno['error'] == 0) {
            $xml = simplexml_load_string($retorno['resposta']);
            return $xml;
        }else{
	...
	{
		$retorno = $this->curl_send('GET',$this->urls['transaction'].'/'.$code, $this->credentials, 30);
        if ($retorno['error'] == 0) {
            $xml = simplexml_load_string($retorno['resposta']);
            return $xml;
        }else{
	...
	{
		$retorno = $this->curl_send('POST',$this->urls['transaction'].'/cancels?'.$code, $this->credentials, 20);
        if ($retorno['error'] == 0) {
            $xml = simplexml_load_string($retorno['resposta']);
            return $xml;
        }else{
    ...
    public function updateOrderStatus($id_cart, $cod_status, $id_order = null, $data_atu = null) 
	{
        ...
        if (isset($status_ps) && $status_ps != false && $current_status != $status_ps) {
			if (in_array(Configuration::get("PS_OS_PAYMENT"), $s_history) != false || in_array(Configuration::get("PAGSEGUROPRO_AUTORIZADO"), $s_history) != false || ((bool)$order->valid === true && $cod_status != 7)) {
				$this->saveLog('status', '', 'Status PagSeguro: '.$this->parseStatus($cod_status), 'Status Loja: '.$current_status);
				return true;
			}else{
				$history = new OrderHistory();
				$history->id_order = (int)$id_order;
				if (!$history->changeIdOrderState($status_ps,(int)$id_order)) {
					$this->saveLog('error', 'Atualiza Status', 'Status PagSeguro: '.$status_ps.' / Status Loja: '.(int)$current_status, 'Status do pedido não atualizado na loja.');
				}
				...
				if (!$this->updatePagSeguroData($data)) {
					$this->saveLog('status', '', 'Status PagSeguro: '.$this->parseStatus($cod_status), 'Status não atualizado na tabela do PagSeguro.');
				}
    			return true;
			}
			return true;
    ...
    private function procDadosEntrega() 
	{
        ...
        $this->ps_params['shippingAddressNumber'] = isset($address->{$this->number_field}) && strlen($address->{$this->number_field}) > 0 ? $address->{$this->number_field} : '1';
		$this->ps_params['shippingAddressComplement'] = $address->{$this->compl_field};
    ...
    private function procDadosCobranca($dados_form) 
	{
        ...
        $this->ps_params['billingAddressNumber'] = isset($dados_form['numero_cobranca']) && $dados_form['numero_cobranca'] != '' ? $dados_form['numero_cobranca'] : $address->{$this->number_field};
		$this->ps_params['billingAddressComplement'] = isset($dados_form['complemento_cobranca']) && $dados_form['complemento_cobranca'] != '' ? $dados_form['complemento_cobranca'] : $address->{$this->compl_field};
	 ...
	 */
    public function curl_send($method, $post_url, array $data = null, $timeout = 10) 
	{
		...
		if ((bool)Configuration::get('PAGSEGUROPRO_FULL_LOG') !== false) {
			$this->saveLog('curl', $method, json_encode($data), utf8_encode(str_replace('ISO-8859-1', 'UTF-8', $resp)), $post_url);
		}
        if ($info['http_code'] == 200 || $info['http_code'] == 201 || $info['http_code'] == 204) {
			...
			$this->saveLog('error', $method, json_encode($data), json_encode($this->ps_errors), $post_url);
			$retorno = array(
				'resposta' =>$resp, 
				'status' => $info['http_code'],
				'info' => $info,
	 ...
	 */
	public function saveLog($type, $method, $data, $response = false, $url = false)
	{
		if (!$response) {
			$response = '';
		}
		...
		$query = 'INSERT INTO `'._DB_PREFIX_.'pagseguropro_logs` (`datetime`, `type`, `method`, `data`, `response`, `url`) VALUES (NOW(), "'.$type.'", "'.$method.'", "'.addslashes($data).'", "'.addslashes($response).'" , "'.addslashes($url).'")';
